...
public class PowerOutagesDAO {
	...
	public List<PowerOutage> loadAllPowerOutages(NercIdMap nIdMap) {
		...
		try {
			...
			while (res.next()) {
				PowerOutage p = new PowerOutage(res.getInt("id"), nIdMap.get(res.getInt("nerc_id")), res.getTimestamp("date_event_began").toLocalDateTime(), res.getTimestamp("date_event_finished").toLocalDateTime());
				powerList.add(p);
			}
